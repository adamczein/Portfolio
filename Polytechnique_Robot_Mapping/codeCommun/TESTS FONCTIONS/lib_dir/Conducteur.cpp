...
void Conducteur::PWM(uint16_t Droite, uint16_t Gauche, Direction sens) {
	...
	switch (sens) {
		case avant:
			PORTD = (1 << 3); // 0x08
			PORTD = (1 << 2); // 0x04
			OCR1A = 0.95*Droite; // 0.89 est un ajustment d�termin� par essaie-erreur
			OCR1B = Gauche;
			break;
		case arriere:
			PORTD = ~(1 << 3); // Inverse pour la polarit�
			PORTD = ~(1 << 2); // Inverse pour la polarit�
			OCR1A = 0.95*Droite; // 0.92 est un ajustment d�termin� par essaie-erreur
			OCR1B = Gauche;
			break;
		case arret:
			PORTD = (1 << 3);
